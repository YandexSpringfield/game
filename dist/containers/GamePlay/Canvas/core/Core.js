import { overworldSprite, tilesSize, } from '@containers/GamePlay/Canvas/sprite-resolver';
...
export class Core {
    constructor(canvasBg, canvasMario) {
        ...
        this.contextMario = this.canvasMario.getContext('2d');
    ...
    init() {
        loadImage(spriteImage)
            ...
            this.mario = new Mario(this.canvasMario, this.sprite, this.contextMario, this.level.matrix);
        })
            .then(() => {
    ...
    updateBackground() {
        ...
        this.contextBg.drawImage(this.canvasBg, -this.camera.pos.x % tilesSize.width, -this.camera.pos.y);
    }
    drawTiles(start, end) {
    ...
    drawCoinsStatus(coins) {
        ...
        this.contextMario.fillText(`Монеты: ${coins}`, this.canvasMario.width - 300, 40);
    }
    drawTimerStatus(time) {
        this.contextMario.clearRect(this.canvasMario.width - 120, 30, 200, 30);
        this.contextMario.font = '20px Roboto';
        this.contextMario.fillStyle = '#fff';
        this.contextMario.fillText(`Время: ${time}`, this.canvasMario.width - 120, 40);
    }
    drawStartText(text) {
        this.contextBg.font = 'bold 70px Roboto';
        this.contextBg.fillStyle = '#fff';
        this.contextBg.textAlign = 'center';
        this.contextBg.fillText(text, this.canvasBg.width / 2, this.canvasBg.height / 2);
        this.contextBg.font = '30px Roboto';
        this.contextBg.fillText('(нажмите ↵ / Enter)', this.canvasBg.width / 2, this.canvasBg.height / 2 + 50);
    }
}
